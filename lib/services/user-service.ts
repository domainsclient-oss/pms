...
const usersCollection = () => getFirestoreDb().collection("users");
...
export async function updateUser(id: string, user: Partial<Omit<User, "id">>) {
  ...
  if (user.name || user.email) {
    await getAdminAuth().updateUser(id, { ...(user.name && { displayName: user.name }), ...(user.email && { email: user.email }) }).catch(() => {});
  }
  return getUser(id);
}
